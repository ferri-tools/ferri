...
impl FlowOrchestrator {
    ...
    pub fn execute(&self) -> io::Result<()> {
        ...
        for wave in execution_order {
            self.execute_wave(&wave, Arc::clone(&job_outputs), &workspace_paths, &update_sender)?;
        }
    ...
    fn process_updates(receiver: crossbeam_channel::Receiver<Update>, job_states: Arc<Mutex<HashMap<String, JobStatus>>>) {
